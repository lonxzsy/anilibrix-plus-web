...
export const useLibraryStore = defineStore('library', () => {
  ...
  async function loadFavorites() {
    ...
    favorites.value = rows.map((r: any) => ({
      id: r.titleId,
      alias: '',
      name: { main: r.titleName, english: '', alternative: '' },
      poster: r.posterUrl ? { src: r.posterUrl, preview: r.posterUrl, thumbnail: r.posterUrl } : undefined,
      year: 0,
      episodesTotal: 0,
      isOngoing: false,
    ...
    if (authStore.token) {
      try {
        ...
        if (timecodes?.data?.length) {
          ...
          const apiHistory = timecodes.data.map((tc: any) => ({
            ...
            watchedAt: tc.updated_at ? Math.floor(new Date(tc.updated_at).getTime() / 1000) : Math.floor(Date.now() / 1000)
          } as HistoryEntry))
  ...
  async function addToHistory(titleId: number, episodeId: string, episodeNumber: number, timestamp: number, duration: number) {
    await db.addHistory({
      titleId,
      episodeId,
      episodeNumber,
  ...
  async function loadPlaylists() {
    ...
    playlists.value = rows.map((r: any) => ({
      id: r.id,
      name: r.name,
