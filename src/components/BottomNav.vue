  ...
  <nav class="bottom-nav glass-strong">
    ...
    >
      <svg class="bottom-nav__item-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path :d="item.iconPath" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" fill="none" />
      </svg>
      <span class="bottom-nav__item-label">{{ item.label }}</span>
    </router-link>

    <button v-if="!authStore.user" class="bottom-nav__item" @click="$emit('openAuth')">
      <svg class="bottom-nav__item-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
        <path d="M15 3h4a2 2 0 012 2v14a2 2 0 01-2 2h-4" />
        <path d="M10 17l5-5-5-5" />
        <path d="M15 12H3" />
      </svg>
      <span class="bottom-nav__item-label">Войти</span>
    </button>
    <router-link v-else to="/profile" class="bottom-nav__item" :class="{ 'bottom-nav__item--active': isActive('/profile') }">
      ...
      <span class="bottom-nav__item-label">{{ authStore.user.name }}</span>
    </router-link>
  </nav>
</template>
...
.bottom-nav {
  ...
  &__indicator {
    ...
    transition:
      ...
      width 280ms var(--md-sys-motion-easing-spring);
    pointer-events: none;
  }
